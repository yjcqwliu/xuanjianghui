...
function fs2_superView()
{
	...
	for(var i=0; i<fs2_data.length ; i++)
	{
		if(fs2_data[i].type == "static")
		{
			...
			{
				html += '<div class="fsg_hy">'+fs2_data[i].real_name+'&nbsp;&nbsp;'+'&nbsp;&nbsp;'+fs2_logo20(fs2_data[i])+'</div>';
			}
		}
		else
		{
...
function fs2_ajax_submit()
{
	...
	var pars = "pars="+fs2_pars+"&text=" + text + "&uid={_UID_}";
	var myAjax = new Ajax.Request(url, {method: "post", parameters: pars, onComplete: function (req) { fs2_ajax_show(req); } });
...
function fs2_ajax_show(req)
{
	...
	{
		html += '<div id=suggest_'+arr[i].id+' class="sgt_of" style="width:200px;cursor:hand" onmouseover="fs2_suggestOnmouseover(this)" onmousedown="fs2_suggestOnmousedown(this);">'+arr[i].real_name+'　'+'&nbsp;&nbsp;'+fs2_logo20(arr[i])+'</div>';
	}
...
function fs2_suggestOnmousedown(thisobj)
{
	...
	fs2_superView();
	*/
	window.location.href = "friend?id="+num;
}
